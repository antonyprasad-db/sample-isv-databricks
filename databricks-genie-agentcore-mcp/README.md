...
[`images/build_architecture.py`](images/build_architecture.py). The Amazon Bedrock, Amazon
Cognito and Amazon CloudWatch marks are from the official
[AWS Architecture Icons](https://aws.amazon.com/architecture/icons/) toolkit; the Amazon
Bedrock AgentCore mark is a separate brand export. To change the diagram, edit
`build_architecture.py` and re-run it (`python build_architecture.py` from `images/`, which
reads the committed `icons_b64.json` icon cache), then regenerate the PNG
(`rsvg-convert -w 2064 architecture.svg -o architecture.png`) — the README embeds the PNG.</sub>
